...
with open('frontend/src/pages/Student/AttendanceLanding.jsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
export default function AttendanceLanding() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setStatus('success');
      setTimeout(() => {
        navigate(/student/feedback/, { state: { session: res.data } });
      }, 1500);
    } catch (err) {
      setStatus('error');
...
old_body = '''          <h2 className="text-2xl font-bold text-white mb-3">Attendance Session</h2>
          <p className="text-slate-300 text-sm mb-8 leading-relaxed">
            You've scanned an attendance QR code. To mark your attendance securely, please open the in-app scanner.
          </p>
...
new_body = '''          {status === 'idle' || status === 'verifying' ? (
            <>
...
with open('frontend/src/pages/Student/AttendanceLanding.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated AttendanceLanding.jsx")
